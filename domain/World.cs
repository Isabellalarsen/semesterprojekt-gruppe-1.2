...
namespace Domain
{
  class World
  {
    ...
    public World()
    {
      ...
      string bikubeText = File.ReadAllText(@"./data/beehive.txt");
      string aviayaText = File.ReadAllText(@"./data/aviaya.txt");
      string jordbundText = File.ReadAllText(@"./data/jordbund.txt");
      string fruHansenText = File.ReadAllText(@"./data/fruhansen.txt");
      string drivhusText = File.ReadAllText(@"./data/drivhus.txt");
      string solbergText = File.ReadAllText(@"./data/solberg.txt");
      string vandpytText = File.ReadAllText(@"./data/vandpyt.txt");
      string eventyrhavenText = File.ReadAllText(@"./data/eventyr.txt");
      string blomsterbedText = File.ReadAllText(@"./data/blomsterbed.txt");
      string vejenText = File.ReadAllText(@"./data/vejen.txt");
      ...
      Question aviayaQuestion =
        new Question(
          "Hvad er bedst for en bi?\n a: Nyslået græs?\n b: Eksotiske planter fra andre lande?\n c: Indfødte planter, der blomstrer på forskellige tidspunkter?    ",
          'c', 20, false);
      Question fruHansenQuestion = new Question(
        "Hvorfor er det vigtigt at undgå brug af pesticider i haver?\n a: Fordi pesticider dræber bier og andre gavlige insekter\n b: Fordi pesticider gør planterne mindre grønne\n c: Fordi bier ikke kan lide lugten af pesticider  ",
        'a', 20, false);
      Question solbergQuestion =
        new Question(
          "Hvilken type vand foretrækker bier i sommervarmen?\n a: Friskt, koldt og rent vand\n b: Surt, og smålugtet vand med jord, mos og blade\n c: Vand med isterninger og citron ",
          'b', 20, false);
      Question eventyrhaveQuestion =
        new Question(
          "Hvad er godt at bestøve for en bi?\n a: Rhododendron, Olieplanter og Balsamin Planter\n b: Planter med pesticidforurening\n c: Solhat, Valmue og bærbuske ",
          'c', 20, false);
      Question vejenQuestion =
        new Question("Hvor vil en bi helst bo?\n a: Eng\n b: Et lækkert hus med spa\n c: På mars", 'a', 20, false);
